# Simple ToDo CRUD (jQuery example)

Overview

This small demo implements a minimal ToDo list UI using jQuery. It intentionally stores the ToDo items in the HTML (table rows) rather than in a JavaScript array to demonstrate event-binding behavior.
...
- `todoscript.js` — jQuery script that handles adding new todos by inserting rows into the table and binding events. New rows are created without delete event handlers on purpose.
